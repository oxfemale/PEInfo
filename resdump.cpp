...
static const TCHAR cszLanguage[] =  "Lang";
static const TCHAR cszString[] =  "String value";
static const TCHAR cszType[] =  "Type";
static const TCHAR cszInfo[] =  "Information";
					 ...
					 )
{
	...
	TCHAR szResID[32];
	TCHAR szLang[16]; wsprintf(szLang,  "0x%04X", langId);
	TCHAR szDump[1024];
	...
	wsprintf(szDump,  "  LangID: 0x%04X,    BaseStringID: %5u.    ResourceSize: %5ubytes.  (Strings trimmed to 512 chars)\n", langId, id, resSize);
	printf(szDump, -1, TRUE);
	printf( "  ----------------------------------------------------------------------------------------------------\n", -1, TRUE);
	...
	for ( unsigned j = 0; j < 16; j++ )
	{
		...
		else
		{
			...
			wsprintf(szResID,  "%u.%s", id + j, szLang );
			...
			*pszDst = 0;
			...
			wsprintf(szDump,  "    %-5u: len=%-4u; ", id + j, len0);
			_tprintf(szDump);
			_tprintf(" %s\r\n",pszBuf);
			printf( "\n");
		}
...
void ParseVersionInfoFixed(  char* fileNameBuffer,   LPVOID ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy, VS_FIXEDFILEINFO* pVer, wchar_t* &psz)
{
...
void ParseVersionInfoVariableString(  char* fileNameBuffer,   LPVOID ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy, LPWORD pToken, wchar_t* &psz)
{
...
void ParseVersionInfoVariableStringA(  char* fileNameBuffer,   LPVOID ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy, char* pToken, wchar_t* &psz)
{
...
void ParseVersionInfoVariableVar(  char* fileNameBuffer,   LPVOID ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy, LPWORD pToken, wchar_t* &psz)
{
...
void ParseVersionInfoVariable(  char* fileNameBuffer,   LPVOID ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy, LPWORD pToken, wchar_t* &psz)
{
	StringFileInfo *pSFI = (StringFileInfo*)pToken;
	...
	if (_wcsicmp(pSFI->szKey, L"StringFileInfo") == 0) {
		LPWORD pEnd = (LPWORD)(((LPBYTE)ptrRes)+resSize);
		if (pToken < pEnd && *pToken > sizeof(StringFileInfo)) {
			ParseVersionInfoVariableString(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy, pToken, psz);
		}
	} else if (_wcsicmp(pSFI->szKey, L"VarFileInfo") == 0) {
		LPWORD pEnd = (LPWORD)(((LPBYTE)ptrRes)+resSize);
		if (pToken < pEnd && *pToken > sizeof(VarFileInfo)) {
			ParseVersionInfoVariableVar(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy, pToken, psz);
		}
	}
}
void ParseVersionInfoVariableA(  char* fileNameBuffer,   LPVOID ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy, char* pToken, wchar_t* &psz)
{
	StringFileInfoA *pSFI = (StringFileInfoA*)pToken;
	...
	if (_stricmp(pSFI->szKey, "StringFileInfo") == 0) {
		char* pEnd = (char*)(((LPBYTE)ptrRes)+resSize);
		if (pToken < pEnd && *pToken > sizeof(StringFileInfo)) {
			ParseVersionInfoVariableStringA(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy, pToken, psz);
		}
	}
}

void ParseVersionInfo(  char* fileNameBuffer,   LPVOID &ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy)
{
		...
		if (pVer->dwSignature == 0xfeef04bd && nNewSize > resSize) {
			...
			if (!ptrBuf) {
				...
				*psz++ = nBOM;
				...
				// VS_FIXEDFILEINFO
				ParseVersionInfoFixed(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy, pVer, psz);
				...
				while (pToken < pEnd && *pToken > sizeof(StringFileInfo)) {
					...
					if (pSFI->wLength == 0)
						break; // Invalid
					ParseVersionInfoVariable(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy, pToken, psz);
					//
...
// ANSI for 16bit PE's
void ParseVersionInfoA(  char* fileNameBuffer,   LPVOID &ptrRes, DWORD &resSize, LPBYTE &ptrBuf, BOOL &bIsCopy)
{
	WORD nTestSize = ((WORD*)ptrRes)[0];
	//WORD nTestShift = ((WORD*)ptrRes)[1]; // ???
			...
			if (!ptrBuf) {
				...
				*psz++ = nBOM;
				...
				while (pToken < pEnd && *pToken > sizeof(StringFileInfoA)) {
					...
					if (pSFI->wLength == 0)
						break; // Invalid
					ParseVersionInfoVariableA(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy, pToken, psz);
					//
						   ...
						   LPCSTR asID, LPCSTR langID, DWORD stringIdBase, DWORD anLangId)
{
	...
	case (WORD)RT_GROUP_CURSOR: case 0x8000+(WORD)RT_GROUP_CURSOR:
		{
			...
			if (!ValidateMemory(ptrRes, max(resSize,sizeof(GRPICONDIR)))) {
				...
				//pChild->SetColumns(((rootType & 0x7FFF)==(WORD)RT_GROUP_ICON) ?  "GROUP_ICON") :  "GROUP_CURSOR",  "!!! Invalid size of resource"));
				...
				wsprintf(szInfo,  "Count: %i", nResCount);
				_tprintf(" %s %s\r\n",((rootType & 0x7FFF)==(WORD)RT_GROUP_ICON) ?  _T("GROUP_ICON") :  _T("GROUP_CURSOR"), szInfo);

				wsprintf(szTextInfo,  "%s, Count: %u\n%s\n",
					((rootType & 0x7FFF)==(WORD)RT_GROUP_ICON) ? _T("GROUP_ICON") : _T("GROUP_CURSOR"),
					nResCount,
					((rootType & 0x7FFF)==(WORD)RT_GROUP_ICON) ? _T("====================") : _T("======================"));
				_tprintf(" %s ",szTextInfo); // Не добавлять в DUMP.TXT		
	...
	case (WORD)RT_STRING:
		{
			...
			__try {
				DumpStringTable(     stringIdBase, anLangId, (LPCWSTR)ptrRes, resSize );
	...
	case (WORD)RT_VERSION:
		{
			if (resSize > sizeof(VS_FIXEDFILEINFO)) {
				ParseVersionInfo(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy);
			}
		} break;
	case 0x8000+(WORD)RT_VERSION:
		{
			if (resSize > sizeof(VS_FIXEDFILEINFO)) {
				ParseVersionInfoA(  fileNameBuffer,   ptrRes, resSize, ptrBuf, bIsCopy);
			}
		} break;
	default:
	...
	printf(" %s \r\n", fileNameBuffer);
	if (szType[0]) {
		_tprintf(" cszType: %s cszInfo: %s\r\n", cszType, szInfo[0] ? szInfo : NULL);
		
	}

	if (!bDontSetData)
		_tprintf(" %s [%d]\r\n",(LPBYTE)ptrRes, resSize);
    ...
    DWORD level, DWORD rootType, DWORD parentType )
{
	...
	//	MPanelItem* pStr = pRoot->Root()->FindChild( "String Table", 0);
	//	if (!pStr) {
	//		pStr = pRoot->Root()->AddFolder( "String Table"));
...
template <class T> void DumpResourceSection(  PBYTE pImageBase, T * pNTHeader)	// 'T' = PIMAGE_NT_HEADERS 32/64
{
	...
	//if ( g_cStrResEntries )
	//{
	//	MPanelItem* pStr = pRoot->AddFolder( "String Table"));
	//	pStr->printf( "<String Table>\n" );
